...
void Visuals::think( ) {
	...
	if( g_menu.main.visuals.noscope.get( )
		...
		&& (g_cl.m_local->GetActiveWeapon( )->m_iItemDefinitionIndex( ) == Weapons_t::AWP || g_cl.m_local->GetActiveWeapon()->m_iItemDefinitionIndex() == Weapons_t::SCAR20
			|| g_cl.m_local->GetActiveWeapon()->m_iItemDefinitionIndex() == Weapons_t::SSG08 || g_cl.m_local->GetActiveWeapon()->m_iItemDefinitionIndex() == Weapons_t::G3SG1)
...
void Visuals::spread_crosshair() {
	...
	if (type == Weapons_t::KNIFE_CT || type == Weapons_t::KNIFE_T || type == Weapons_t::C4 || type == Weapons_t::FLASHBANG
		|| type == Weapons_t::HEGRENADE || type == Weapons_t::MOLOTOV || type == Weapons_t::SMOKE || type == Weapons_t::DECOY
		|| type == Weapons_t::FIREBOMB)
		return;

	static float rot = 0.f;

	int w = g_cl.m_width, h = g_cl.m_height;
	...
	int r, g, b;
	r = g_menu.main.visuals.spread_xhair_col.get().r();
	g = g_menu.main.visuals.spread_xhair_col.get().g();
	b = g_menu.main.visuals.spread_xhair_col.get().b();

	circle_dual_colour(w, h, weapon->GetInaccuracy() * 500.0f, 0, 1, 50, D3DCOLOR_RGBA(r, g, b, 100), D3DCOLOR_RGBA(0, 0, 0, 0), g_csgo.m_device);
...
{
	name.erase(name.find("weapon_"), 7);
	return name;
}
void Visuals::DrawItem( Weapon* item ) {
	...
	int type = item->m_iItemDefinitionIndex();
	if (type == Weapons_t::KNIFE_CT || type == Weapons_t::KNIFE_T || type == Weapons_t::C4 || type == Weapons_t::FLASHBANG
		|| type == Weapons_t::HEGRENADE || type == Weapons_t::MOLOTOV || type == Weapons_t::SMOKE || type == Weapons_t::DECOY
		|| type == Weapons_t::FIREBOMB)
		return;
