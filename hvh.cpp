...
void HVH::AntiAim() {
	...
	m_yaw_fake = g_menu.main.antiaim.fake_yaw.get();
	...
	m_rand_update = g_menu.main.antiaim.rot_range_update.get();
	m_dir = g_menu.main.antiaim.yaw_direction.get();
	m_base_angle = g_menu.main.antiaim.yaw_base.get();
	//m_auto_time = g_menu.main.antiaim.yaw_auto_time.get();
...
void HVH::PrepareFake() {
	if (g_cl.m_ground) {
		if ((g_menu.main.antiaim.rage_aa_real_around_fake_standing.get() && g_cl.m_speed < 3.5f) || (g_menu.main.antiaim.rage_aa_real_around_fake_moving.get() && g_cl.m_speed > 3.5f && !g_movement.m_slow_motion) || (g_menu.main.antiaim.rage_aa_real_around_fake_slow_motion.get() && g_movement.m_slow_motion)) {
		...
		if (g_menu.main.antiaim.rage_aa_real_around_fake_air.get()) {
			m_use_real_around_fake = true;
		}
		else {
			m_use_real_around_fake = false;
	...
	// set lby angle in the choked commands.
		...
		if (!m_use_real_around_fake) {
			...
			float yaw = 120.f;
			if (g_menu.main.antiaim.rage_aa_real_around_fake_jitter.get()) {
...
float HVH::ChooseReal() {
	...
	if (m_yaw > 0) {
		...
		if (g_menu.main.antiaim.jitter_type.get() > 0) {
			// get the range from the menu.
			float range = m_jitter_range / 2.f;

			// offset.
			if (g_menu.main.antiaim.jitter_type.get() == 1) {
		...
		if (g_menu.main.antiaim.rot_enable.get()) {
			// apply spin.
...
void HVH::AdjustYaw() {
	...
	float custom_fake = std::clamp<float>(g_menu.main.antiaim.rage_aa_custom_fake.get(), -60.f, 60.f);
...
void HVH::SendPacket() {
	...
	if (g_menu.main.antiaim.lag_enable.get() && !g_csgo.m_gamerules->m_bFreezePeriod() && !(g_cl.m_flags & FL_FROZEN)) {
		// limit of lag.
		int limit = std::clamp<int>((int)g_menu.main.antiaim.lag_limit.get(), 2, g_cl.m_max_lag);
		...
		for (auto it = activation.begin(); it != activation.end(); it++) {
			...
			// move.
			if (*it==1 && delta > 0.1f && g_cl.m_speed > 0.1f && !m_fake_duck) {
				g_cl.m_should_lag = true;
			}

			// air.
			else if (*it==2 && !m_fake_duck && ((g_cl.m_buttons & IN_JUMP) || !(g_cl.m_flags & FL_ONGROUND))) {
				g_cl.m_should_lag = true;
			}

			// crouch.
			else if (*it==3 && g_cl.m_local->m_bDucking() && !m_fake_duck) {
				g_cl.m_should_lag = true;
			}
	...
	if (!g_hvh.m_fake_duck && g_menu.main.antiaim.lag_mode.get() == 0) {
		if (g_cl.m_old_shot)
			*g_cl.m_packet = true;
	}
