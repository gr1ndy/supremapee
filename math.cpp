...
void math::AngleVectors_2(const vec3_t& angles, vec3_t* forward)
{
#define Assert(_exp) ((void)0)
    ...
    sy = std::sin(math::deg_to_rad(angles[1]));
    cy = std::cos(math::deg_to_rad(angles[1]));

    sp = std::sin(math::deg_to_rad(angles[0]));
    cp = std::cos(math::deg_to_rad(angles[0]));
