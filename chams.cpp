...
void zMatrixCopy(const matrix3x4_t& source, matrix3x4_t& target)
{
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 4; j++) {
...
void zVectorRotate(const vec3_t& in1, const matrix3x4_t& in2, vec3_t& out)
{
	zzVectorRotate(&in1.x, in2, &out.x);
}


void zAngleMatrix(const vec3_t angles, matrix3x4_t& matrix)
{
...
void VectorRotate(const vec3_t& in1, const vec3_t& in2, vec3_t& out)
{
	matrix3x4_t matRotate;
	zAngleMatrix(vec3_t(in2.x, in2.y, in2.z), matRotate);
...
void Chams::RenderFakeChams() {
	...
	if (!g_menu.main.players.esp_chams_desync_enable.get())
		return;

	// get color.
	Color color = g_menu.main.players.esp_chams_desync_color.get();
	...
	if (true)
	{
		...
		for (int i = 0; i < 128; i++)
		{
			g_cl.m_fake_state = g_cl.m_local->m_PlayerAnimState();
			if (g_cl.m_fake_state)
				zAngleMatrix(vec3_t(0, g_cl.m_fake_state->m_goal_feet_yaw, 0), g_cl.m_fake_matrix[i]);
			MatrixMultiply(g_cl.m_fake_matrix[i], g_cl.bone_to_world[i]);
			BonePos = vec3_t(g_cl.bone_to_world[i][0][3], g_cl.bone_to_world[i][1][3], g_cl.bone_to_world[i][2][3]) - g_cl.model_origin;
			VectorRotate(BonePos, vec3_t(0, g_cl.m_angle.y, 0), OutPos);
			OutPos += g_cl.model_origin;
			g_cl.m_fake_matrix[i][0][3] = OutPos.x;
			g_cl.m_fake_matrix[i][1][3] = OutPos.y;
			g_cl.m_fake_matrix[i][2][3] = OutPos.z;
		}
	...
	if (g_cl.m_fake_matrix) {
		...
		// set material and color.
		SetupMaterial(g_chams.debugambientcube, color, true);
