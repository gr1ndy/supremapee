...
#define IMGUI_CHECKVERSION()        ui::DebugCheckVersionAndDataLayout(IMGUI_VERSION, sizeof(ImGuiIO), sizeof(ImGuiStyle), sizeof(ImVec2), sizeof(ImVec4), sizeof(ImDrawVert))
...
#ifndef IM_ASSERT
#include <assert.h>
#define IM_ASSERT(_EXPR)            if (!(_EXPR)) exit(0)                             // You can override the default assert handler by editing imconfig.h
#endif
...
namespace ui
{
    ...
    IMGUI_API ImU32         GetColorU32(const color_t* col);
    ...
    IMGUI_API bool          ListBox(int fix, const char* szLabel, int* iCurrentItem, std::function<const char* (int)> pLambda, int nItemsCount, int iHeightInItems);
    IMGUI_API bool          TestListBox(const char* szLabel, int* iCurrentItem, std::function<const char* (int)> pLambda, int nItemsCount, int iHeightInItems);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, int items = 1);
    IMGUI_API void          EndCombo(); // only call EndCombo() if BeginCombo() returns true!
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* const items[], int items_count, int popup_max_height_in_items = -1);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items = -1);      // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int popup_max_height_in_items = -1);
    bool SingleSelect(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), int items_count);
    IMGUI_API bool          SingleSelect(const char* label, int* current_item, std::vector<const char*> items);
    IMGUI_API bool MultiSelectRefix(const char* label, std::vector<bool> _data, std::vector<const char*> items, int fix);
	IMGUI_API bool          MultiSelect(const char* label, bool _data[], std::vector<const char*> items, int fix);
    ...
    IMGUI_API bool          SliderInt(const char* label, int* v, int v_min, int v_max, const char* format = "%d", int remove_from_fmt = 0);
    IMGUI_API bool          SliderInt2(const char* label, int v[2], int v_min, int v_max, const char* format = "%d");
    IMGUI_API bool          SliderInt3(const char* label, int v[3], int v_min, int v_max, const char* format = "%d");
    IMGUI_API bool          SliderInt4(const char* label, int v[4], int v_min, int v_max, const char* format = "%d");
    IMGUI_API bool          SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format = NULL, float power = 1.0f, int remove_from_fmt = 0);
    ...
    IMGUI_API bool          ListBox(int fix, const char* label, int* current_item, const char* const items[], int items_count, int height_in_items = -1);
    IMGUI_API bool          ListBox(int fix, const char* label, int* current_item, bool (*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int height_in_items = -1);
    bool ListBoxHeader(int fix, const char* label, const ImVec2& size_arg);
    bool ListBoxHeader(int fix, const char* label, int items_count, int height_in_items);
    ...
    IMGUI_API bool          Keybind(const char* id, int* current_key, int* key_style = NULL);
    ...
    IMGUI_API void          TabButton(const char* label, int* currenttab, int number, int total);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_ChildFrame              = 1 << 29,
	ImGuiWindowFlags_Visuals				= 1 << 30
...
struct ImVector
{
    ...
    inline ~ImVector()                                      { if (Data) ui::MemFree(Data); }
    ...
    inline void         clear()                             { if (Data) { Size = Capacity = 0; ui::MemFree(Data); Data = NULL; } }
    ...
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; T* new_data = (T*)ui::MemAlloc((size_t)new_capacity * sizeof(T)); if (Data) { memcpy(new_data, Data, (size_t)Size * sizeof(T)); ui::MemFree(Data); } Data = new_data; Capacity = new_capacity; }
...
#define IM_NEW(_TYPE)                       new(ImNewDummy(), ui::MemAlloc(sizeof(_TYPE))) _TYPE
...
struct ImDrawList
{
	...
	IMGUI_API void  AddTextShadow(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
