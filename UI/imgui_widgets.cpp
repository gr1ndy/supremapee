...
bool ui::ListBox(int fix, const char* szLabel, int* iCurrentItem, std::function<const char* (int)> pLambda, int nItemsCount, int iHeightInItems)
{
    return ListBox(fix, szLabel, iCurrentItem, [](void* pData, int nIndex, const char** szOutText)
        {
            *szOutText = (*(std::function<const char* (int)>*)pData)(nIndex);
            return true;
        }, &pLambda, nItemsCount, iHeightInItems);
}
bool ui::TestListBox(const char* szLabel, int* iCurrentItem, std::function<const char* (int)> pLambda, int nItemsCount, int iHeightInItems)
{
   // return ListBox(fix, szLabel, iCurrentItem, [](void* pData, int nIndex, const char** szOutText)
   //     {
   //         *szOutText = (*(std::function<const char* (int)>*)pData)(nIndex);
...
bool ui::_SingleSelect(const char* label, int* current_item, std::vector<std::string> items) {
	*current_item = ImClamp(*current_item, 0, int(items.size() - 1));

	int old_item = *current_item;

	if (ui::BeginCombo(label, items.at(*current_item).c_str(), ImGuiComboFlags_HeightLargest, items.size())) {
		for (int i = 0; i < items.size(); i++)
			if (ui::Selectable(items.at(i).c_str(), *current_item == i))
				* current_item = i;
...
#include <string>
static std::string prevValue[9999];



bool ui::MultiSelectRefix(const char* label, std::vector<bool> _data, std::vector<const char*> items, int fix)
{
    //std::unordered_map<int, bool>
    //std::unordered_map<int, bool> old_data = *data;
    std::unordered_map<int, bool> old_data;
    std::unordered_map<int, bool> _old_data;
    for (int i = 0; i < items.size(); ++i)
        old_data[i] = _data[i];
   ...
   // }
    ...
    //prevValue[fix] = "";
    {
        if (ui::BeginCombo(label, prevValue[fix].c_str(), 0, items.size()))
        {
            prevValue[fix] = "";
            std::vector<std::string> vec;
            for (auto i = 0; i < items.size(); ++i) {
                ui::Selectable(items.at(i), &_data[i], ImGuiSelectableFlags_DontClosePopups);
                if (_data[i]) vec.push_back(items.at(i));
            }
            for (size_t i = 0; i < vec.size(); i++)
            {
                if (vec.size() == 1)
                    prevValue[fix] += vec.at(i);
                else if (i != vec.size())
                    prevValue[fix] += vec.at(i) + ", ";
                else
                    prevValue[fix] += vec.at(i);
            }
            if (prevValue[fix].size() >= 2)
            {
                std::string temp(prevValue[fix], (prevValue[fix].size() - 2), 2);
                if (!temp.compare(", ")) prevValue[fix] = prevValue[fix].substr(0, prevValue[fix].size() - 2); //fix space ,
            }
            ui::EndCombo();
        }
    }
...
bool ui::MultiSelect(const char* label, bool _data[], std::vector<const char*> items, int fix)
{
	//std::unordered_map<int, bool>
	//std::unordered_map<int, bool> old_data = *data;
	std::unordered_map<int, bool> old_data;
...
bool ui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt)
{
    ...
    if (value_changed)
     ...
     window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), skeet_menu.get_accent_color());
    ...
    {
        PushFont(GetIO().Fonts->Fonts[1]);
        window->DrawList->AddText(ImVec2(slider_bb.Min.x + 10 + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)) - textSize.x / 2, slider_bb.Max.y - 4 - textSize.y / 2 - 1), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), value_buf);
        PopFont();
    }
...
{
    const bool value_changed = ListBox(0, label, current_item, Items_ArrayGetter, (void*)items, items_count, height_items);
    return value_changed;
}

bool ui::ListBox(int fix, const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
...
bool ui::InputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, buf, (int)buf_size, ImVec2(0, 15), flags, callback, user_data);
}
