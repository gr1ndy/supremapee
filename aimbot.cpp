...
void AimPlayer::OnNetUpdate( Player *player ) {
	...
	if ( !update && !player->dormant( ) && player->m_vecOrigin( ) != player->m_vecOldOrigin( ) ) {
		...
		// fix data.
		player->m_flSimulationTime( ) = game::TICKS_TO_TIME( g_csgo.m_cl->clock_drift_mgr.m_server_tick);
	}
...
void Aimbot::find( ) {
	...
	if ( best.player && best.record ) {
		...
		// if we can scope.
		bool can_scope = !g_cl.m_local->m_bIsScoped( ) && (g_cl.m_weapon_id == Weapons_t::AWP || g_cl.m_weapon_id == Weapons_t::SCAR20
			|| g_cl.m_weapon_id == Weapons_t::SSG08 || g_cl.m_weapon_id == Weapons_t::G3SG1	);
...
bool AimPlayer::GetBestAimPosition( vec3_t &aim, float &damage, LagRecord *record ) {
	...
	for ( const auto &it : m_hitboxes ) {
		...
		for ( const auto &point : points ) {
			...
			record->m_player->m_BoneCache2() = reinterpret_cast<matrix3x4_t**>(record->m_bones);
...
void Aimbot::apply( ) {
	...
	if ( attack || attack2 ) {
		...
		*g_cl.m_packet = false;
		...
		// store fired shot.
		g_shots.OnShotFire( m_target ? m_target : nullptr, m_target ? m_damage : -1.f, g_cl.m_weapon_info->iBullets, m_target ? m_record : nullptr );
